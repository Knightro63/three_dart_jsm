...
final _face = new three.Triangle();
final _color = new three.Vector3();
...
class MeshSurfaceSampler {
  ...
  MeshSurfaceSampler setWeightAttribute( name){
  ...
  MeshSurfaceSampler build() {
    ...
    final faceWeights = new Float32List( positionAttribute.count ~/ 3 ); // Accumulate weights for each mesh face.
  ...
  MeshSurfaceSampler sample( targetPosition, targetNormal, targetColor ) {
  ...
  int binarySearch( x ) {
    ...
    while(start <= end){
      final mid = three.Math.ceil( ( start + end ) / 2 );
  ...
  MeshSurfaceSampler sampleFace( faceIndex, targetPosition, targetNormal, targetColor ) {
