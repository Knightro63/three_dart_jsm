...
late Group sceneGraph;

class FBXLoader extends Loader {
  late int innerWidth;
  late int innerHeight;

  FBXLoader(LoadingManager? manager, this.innerWidth, this.innerHeight) : super(manager);
  ...
  dynamic load(url, Function onLoad, [Function? onProgress, Function? onError]) {
    ...
    final path = (scope.path == '') ? LoaderUtils.extractUrlBase(url) : scope.path;
    ...
    loader.load(url, (buffer) {
      onLoad(scope._parse(buffer, path));
    }, onProgress, onError);
  }

  Future<Object3D> _parse(dynamic FBXBuffer, [String path = '', Function? onLoad, Function? onError]) {
    print(FBXBuffer is Uint8List);
    if (_isFbxFormatBinary(FBXBuffer)) {
      _fbxTree = _BinaryParser().parse(FBXBuffer);
    } 
    else {
      final FBXText = _convertArrayBufferToString(FBXBuffer);
    ...
    return __FBXTreeParser(textureLoader, this.manager, this.innerWidth, this.innerHeight).parse();
  }
...
class __FBXTreeParser {
  ...
  __FBXTreeParser(TextureLoader textureLoader, manager, int innerWidth, int innerHeight) {
  ...
  Future<Object3D> parse() async {
    connections = this.parseConnections();
    final images = this.parseImages();

    final textures = await this.parseTextures(images);
  ...
  Future<Map<int,Texture>> parseTextures(images) async {
    ...
    if (_fbxTree.Objects["Texture"] != null) {
      ...
      for (final nodeID in textureNodes.keys) {
        final texture = await this.parseTexture(textureNodes[nodeID], images);
  ...
  Future<Texture> parseTexture(textureNode, images) async {
    final Texture texture = await this.loadTexture(textureNode, images);
  ...
  Material? parseMaterial(Map<String, dynamic> materialNode, Map textureMap) {
    ...
    Map<String,dynamic> parameters = this.parseParameters(materialNode, textureMap, ID);
    ...
    switch (type.toLowerCase()) {
      ...
      default:
        print(
            'FBXLoader: unknown material type "%s". Defaulting to MeshPhongMaterial.${type}');
        material = MeshPhongMaterial();
        break;
    }
    ...
    if (connection["children"] != null) {
      connection["children"].forEach((child) {
        ...
        switch (type) {
          ...
          default:
            print(
                'FBXLoader: %s map is not supported in three.js, skipping texture. ${type}');
            break;
        }
      });
    }
  ...
  Map<String, dynamic> parseSkeleton(relationships, deformerNodes) {
    ...
    relationships["children"].forEach((child) {
      ...
      final rawBone = {
        ...
        "weights": [],
        "transformLink":
            Matrix4().fromArray(boneNode["TransformLink"]["a"]),
  ...
  parseScene(deformers, Map geometryMap, Map materialMap) {
    ...
    final animations = _AnimationParser().parse();

    // if all the models where already combined in a single group, just return that
    if (sceneGraph.children.length == 1 && sceneGraph.children[0] is Group) {
      (sceneGraph.children[0] as Group).animations = animations;
      sceneGraph = sceneGraph.children[0] as Group;
    }
  ...
  Map parseModels(skeletons, geometryMap, materialMap) {
    ...
    for (final nodeID in modelNodes.keys) {
      ...
      Object3D? model = this.buildSkeleton(relationships, skeletons, id, node["attrName"]);
  ...
  Object3D? createCamera(relationships) {
    ...
    if (cameraAttribute == null) {
      ...
      num nearClippingPlane = 1;
      if (cameraAttribute.NearPlane != null) {
        nearClippingPlane = cameraAttribute.NearPlane.value / 1000;
      }

      num farClippingPlane = 1000;
  ...
  createLight(relationships) {
    ...
    if (lightAttribute == null) {
      ...
      switch (type) {
        ...
        default:
          print(
              'FBXLoader: Unknown light type ${lightAttribute.LightType.value}, defaulting to a PointLight.');
  ...
  Line createCurve(relationships, geometryMap) {
    ...
    final material = LineBasicMaterial({"color": 0x3300ff, "linewidth": 1});
  ...
  void getTransformData(model, Map modelNode) {
    ...
    if (modelNode["InheritType"] != null)
      transformData["inheritType"] =
          _parseInt(modelNode["InheritType"]["value"]);

    if (modelNode["RotationOrder"] != null)
      transformData["eulerOrder"] =
          _getEulerOrder(modelNode["RotationOrder"]["value"]);
    else
  ...
  parsePoseNodes() {
    ...
    if (_fbxTree.Objects.keys.contains("Pose")) {
  ...
  void createAmbientLight() {
    if (_fbxTree.GlobalSettings != null &&
        _fbxTree.GlobalSettings!["AmbientColor"] != null) {
...
class _GeometryParser {
  ...
  BufferGeometry genGeometry(Map geoNode, skeleton, morphTargets, preTransform) {
    ...
    if (buffers["colors"].length > 0) {
      geo.setAttribute(
          'color', Float32BufferAttribute(buffers["colors"], 3));
    }
    ...
    if (buffers["normal"].length > 0) {
      final normalMatrix = Matrix3().getNormalMatrix(preTransform);
  ...
  BufferGeometry parseNurbsGeometry(geoNode) {
    if (NURBSCurve == null) {
      print('FBXLoader: The loader relies on NURBSCurve for any nurbs present in the model. Nurbs will show up as empty geometry.');
...
class _AnimationParser {
          ...
          if (curveNodesMap.containsKey(child["ID"])) {
            ...
            if (curveNode["curves"]["x"] != null ||
              ...
              if (modelID != null) {
                ...
                if (rawModel == null) {
                  print(
                      'FBXLoader: Encountered a unused curve. ${child}');
                  return;
                }
  ...
  generateTracks(Map rawTracks) {
    ...
    final initialPosition = initialPositionVector3.toArray();
    final initialRotation = Euler()
        .setFromQuaternion(initialRotationQuaternion, rawTracks["eulerOrder"])
        .toArray();
    final initialScale = initialScaleVector3.toArray();

    if (rawTracks["T"] != null && rawTracks["T"]["curves"].keys.length > 0) {
      final positionTrack = this._generateVectorTrack(rawTracks["modelName"],
    ...
    if (rawTracks["R"] != null && rawTracks["R"]["curves"].keys.length > 0) {
      final rotationTrack = this.generateRotationTrack(
          rawTracks["modelName"],
          rawTracks["R"]["curves"],
          initialRotation,
    ...
    if (rawTracks["S"] != null && rawTracks["S"]["curves"].keys.length > 0) {
      final scaleTrack = this._generateVectorTrack(rawTracks["modelName"],
  ...
  VectorKeyframeTrack _generateVectorTrack(String modelName, Map curves, initialValue, type) {
    final times = this._getTimesForAllAxes(curves);
    final values = this._getKeyframeTrackValues(times, curves, initialValue);
  ...
  QuaternionKeyframeTrack generateRotationTrack(String modelName, Map curves, List<num> initialValue, preRotation, postRotation, eulerOrder) {
    ...
    for (int i = 0; i < values.length; i += 3) {
      euler.set(values[i].toDouble(), values[i + 1].toDouble(), values[i + 2].toDouble(), eulerOrder);
  ...
  void interpolateRotations(Map<String,dynamic> curve) {
    for (int i = 1; i < curve["values"].length; i++) {
      ...
      final absoluteSpan = Math.abs(valuesSpan);
...
class _TextParser {
  ...
  _FBXTree parse(String text) {
    ...
    split.asMap().forEach((i, line) {
      ...
      final matchBeginning = line.startsWith('^\\t{' + scope.currentIndent.toString() + '}(\\w+):(.*){');
      final matchProperty = line.contains('^\\t{' +(scope.currentIndent.toString()) +'}(\\w+):[\\s\\t\\r\\n](.*)');
      final matchEnd = line.endsWith('^\\t{' + (scope.currentIndent - 1).toString() + '}}');
  ...
  void parseNodeBegin(String line, List<String> property) {
    ...
    final _nodeAttrs = property[2].split(',').map((attr) {
      return attr
          .trim()
          .replaceFirst(RegExp(r'^"'), '')
          .replaceAll(RegExp(r'"$'), '');
    ...
    final attrs = this.parse_NodeAttr(_nodeAttrs);
  ...
  _NodeAttr parse_NodeAttr(List<String> attrs) {
  ...
  parseNodeProperty(String line, List<String> property, String contentLine) {
    ...
    final _regExp2 = RegExp(r'"$');

    String propName =
        property[1].replaceFirst(_regExp, '').replaceFirst(_regExp2, '').trim();
    String propValue =
        property[2].replaceFirst(_regExp, '').replaceFirst(_regExp2, '').trim();
...
class _BinaryReader {
  ...
  int getUint8() {
    final value = this.dv.buffer.asByteData().getUint8(this.offset);
    this.offset += 1;
    return value;
  }
  ...
  int getInt16() {
    final value = this
        ...
        .buffer
        .asByteData()
        .getInt16(this.offset, this.littleEndian ? Endian.little : Endian.big);
    this.offset += 2;
    return value;
  }
  ...
  int getInt32() {
    final value = this
        ...
        .buffer
        .asByteData()
        .getInt32(this.offset, this.littleEndian ? Endian.little : Endian.big);
    this.offset += 4;
    return value;
  }
  ...
  double getFloat32() {
    final value = this.dv.buffer.asByteData().getFloat32(
  ...
  double getFloat64() {
    final value = this.dv.buffer.asByteData().getFloat64(this.offset, this.littleEndian ? Endian.little : Endian.big);
    this.offset += 8;
    return value;
  }
  ...
  Uint8List getArrayBuffer(int size) {
    final value = this.dv.sublist(this.offset, this.offset + size);
    this.offset += size;
    return value;
  }
...
Matrix4 _generateTransform(Map transformData) {
  ...
  if (transformData["translation"] != null)
    lTranslationM.setPositionFromVector3(tempVec.fromArray(transformData["translation"]));
  ...
  if (transformData["preRotation"] != null) {
    List<double> array = List<double>.from(transformData["preRotation"]
        ...
        .toList());
    array.add(Euler.rotationOrders
  ...
  if (transformData["scalingOffset"] != null)
    lScalingOffsetM
        .setPositionFromVector3(tempVec.fromArray(transformData["scalingOffset"]));
  if (transformData["scalingPivot"] != null)
    lScalingPivotM
        .setPositionFromVector3(tempVec.fromArray(transformData["scalingPivot"]));
  if (transformData["rotationOffset"] != null)
    lRotationOffsetM
        .setPositionFromVector3(tempVec.fromArray(transformData["rotationOffset"]));
  if (transformData["rotationPivot"] != null)
    lRotationPivotM
        .setPositionFromVector3(tempVec.fromArray(transformData["rotationPivot"]));
  ...
  if (inheritType == 0) {
    ...
    final lParentLSM =
        Matrix4().scale(Vector3().setFromMatrixScale(lParentLX));
...
String _getEulerOrder(int? order) {
  ...
  if (order == 6) {
    print(
        'FBXLoader: unsupported Euler Order: Spherical XYZ. Animations and rotations may be incorrect.');
    return enums[0];
  }
