...
final WEBGL_COMPONENT_TYPES_BYTES_PER_ELEMENT = {
...
final ALPHA_MODES = {"OPAQUE": 'OPAQUE', "MASK": 'MASK', "BLEND": 'BLEND'};
...
Future<BufferGeometry> addMorphTargets(BufferGeometry geometry, targets, GLTFParser parser) async {
  ...
  for (int i = 0, il = targets.length; i < il; i++) {
    ...
    if (hasMorphPosition) {
      final _position = target["POSITION"] != null
    ...
    if (hasMorphNormal) {
      final _normal = target["NORMAL"] != null
    ...
    if (hasMorphColor) {
      final _color = target["COLOR_0"] != null
  ...
  if (attributes["POSITION"] != null) {
    ...
    if (min != null && max != null) {
      ...
      if (accessor["normalized"] != null &&
        ...
        final boxScale = getNormalizedComponentScale(
