...
class RGBELoader extends DataTextureLoader {
  ...
  RGBELoader([LoadingManager? manager]) : super(manager) {}

  // adapted from http://www.graphics.cornell.edu/~bjw/rgbe.html

  Map<String,dynamic>? parse(buffer, [String? path, Function? onLoad, Function? onError]) {
    int byteArrayPos = 0;
    ...
    final rgbe_error = (rgbe_error_code, msg) {
    ...
    final RGBE_VALID_PROGRAMTYPE = 1,
        RGBE_VALID_FORMAT = 2,
        RGBE_VALID_DIMENSIONS = 4;

    final NEWLINE = '\n';

    final fgets = (Uint8List buffer, [lineLimit, consume]) {
      final chunkSize = 128;
    ...
    final RGBE_ReadHeader = (buffer) {
      // regexes to parse header info fields
      final magic_token_re = RegExp(r"^#\?(\S+)"),
    ...
    final RGBE_ReadPixels_RLE = (Uint8List buffer, int w, int h) {
      ...
      final data_rgba = Uint8List(4 * w * h);
      ...
      final ptr_end = 4 * scanline_width;
      final rgbeStart = Uint8List(4);
      final scanline_buffer = Uint8List(ptr_end);
      int num_scanlines = h;
      ...
      while ((num_scanlines > 0) && (pos < buffer.lengthInBytes)) {
        ...
        final l = scanline_width; //scanline_buffer.lengthInBytes;
        for (int i = 0; i < l; i++) {
          int off = 0;
    ...
    final RGBEByteToRGBHalf = (sourceArray, sourceOffset, destArray, destOffset) {
      final e = sourceArray[sourceOffset + 3];
      final scale = Math.pow(2.0, e - 128.0) / 255.0;
    ...
    Map<String, dynamic> rgbe_header_info = RGBE_ReadHeader(byteArray) as Map<String, dynamic>;

    if (RGBE_RETURN_FAILURE != rgbe_header_info) {
      rgbe_header_info = rgbe_header_info;

      final w = rgbe_header_info["width"], h = rgbe_header_info["height"];
  ...
  @override
  Future<DataTexture> loadAsync(url) async {
    final Completer<DataTexture> completer = Completer();

    load(url, (result,texData) {
  ...
  load(url, Function? onLoad, [Function? onProgress,Function? onError]) {
    void Function(DataTexture,Map<String,dynamic>) onLoadCallback = (texture, texData) {
      switch (texture.type) {
        case UnsignedByteType:
