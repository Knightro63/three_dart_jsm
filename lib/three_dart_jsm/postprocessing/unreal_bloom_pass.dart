...
import 'pass.dart';
import '../shaders/luminosity_high_pass_shader.dart';
...
class UnrealBloomPass extends Pass {
  ...
  UnrealBloomPass(Vector2? resolution, num? strength, double radius, double threshold): super() {
    ...
    this.renderTargetBright = WebGLRenderTarget(resx, resy, pars);
    this.renderTargetBright.texture.name = 'UnrealBloomPass.bright';
    this.renderTargetBright.texture.generateMipmaps = false;

    for (int i = 0; i < this.nMips; i++) {
    ...
    this.materialHighPassFilter = ShaderMaterial({
    ...
    this.basic = MeshBasicMaterial(Map<String, dynamic>());

    this.fsQuad = FullScreenQuad(null);
  }

  void dispose() {
    for (int i = 0; i < this.renderTargetsHorizontal.length; i++) {
      this.renderTargetsHorizontal[i].dispose();
    }

    for (int i = 0; i < this.renderTargetsVertical.length; i++) {
  ...
  void render(renderer, writeBuffer, readBuffer,{num? deltaTime, bool? maskActive}) {
    ...
    RenderTarget inputRenderTarget = this.renderTargetBright;
