...
class WebGPUUniformsGroup extends WebGPUUniformBuffer {
  ...
  WebGPUUniformsGroup addUniform(uniform) {
    this.uniforms.add(uniform);

    return this;
  }

  WebGPUUniformsGroup removeUniform(uniform) {
  ...
  getBuffer() {
    ...
    if (buffer == null) {
      final byteLength = this.getByteLength();
  ...
  getByteLength() {
    ...
    for (final i = 0, l = this.uniforms.length; i < l; i++) {
      final uniform = this.uniforms[i];

      // offset within a single chunk in bytes

      final chunkOffset = offset % GPUChunkSize;
      final remainingSizeInChunk = GPUChunkSize - chunkOffset;
  ...
  updateMatrix3(uniform) {
    ...
    final a = this.buffer;
    final e = uniform.getValue().elements;
    final offset = uniform.offset;
...
arraysEqual(a, b, offset) {
  for (final i = 0, l = b.length; i < l; i++) {
